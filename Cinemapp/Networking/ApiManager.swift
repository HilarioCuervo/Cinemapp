...
class ApiManager {
    ...
    func performCurrentMovieRequest(completionHandler: @escaping (CurrentMovieModel?, Error?) -> Void) {
        ...
        AF.request(currentMovieUrl).response { response in
            do {
                ...
                if let responseData = response.data {
                    let safeData = try decoder.decode(CurrentMovieModel.self, from: responseData)
